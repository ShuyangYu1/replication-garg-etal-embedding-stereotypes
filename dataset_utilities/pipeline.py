...
def clean_googlenewsvectors():
    with open('../vectors/vectorsGoogleNews.txt', 'r', encoding='utf-8', errors='ignore', newline='') as r:
        with open('../vectors/vectorsGoogleNews_exactclean.txt', 'w', encoding='utf-8', newline='') as wf:
            reader = csv.reader(r, delimiter=' ', quoting=csv.QUOTE_NONE)
            writer = csv.writer(wf, delimiter=' ')
            for row in reader:
                word = row[0]
                if len(word) < 20 and len(re.sub('[^a-z]+', '', word)) == len(word):
                    writer.writerow(row)
