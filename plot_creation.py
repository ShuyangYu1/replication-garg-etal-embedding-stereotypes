...
def plot_averagebias_over_time_consistentoccupations(row, label='', neutral_words='', group1='male', group2='female', overlay_with_occ_percents = False, occ_percents_file = None, occ_func = None, shift=0, limit_to_certain_words = False, limit_words_file = '', ylim1 = None, ylim2 = None, normalize_by_pairsdist = False, pairs_dist_row_file = 'run_results/finalrun.csv', yrs_to_do=None, shift_yrs_plot_labels = 0):
    ...
    # Create dataframe for seaborn lineplot
    n_series = arembed.shape[1]
    df_plot = pd.DataFrame({
        'time': np.repeat(yrs_plot, arembed.shape[0]),
        'value': arembed.flatten(order='F'),  # Flatten column-wise
        'series': np.tile(range(arembed.shape[0]), len(yrs_plot))
    })
    sns.lineplot(data=df_plot, x='time', y='value', 
                estimator=np.nanmean, errorbar=('ci', 68), 
                ax=ax1, legend=False)
    ...
    if overlay_with_occ_percents:
        ...
        sns.lineplot(data=df_plot, x='time', y='value', 
                    estimator=np.nanmean, errorbar=('ci', 68), 
                    ax=ax1, legend=False, color = 'g')#, marker = "o", condition = 'Avg. {}'.format(occ_func.label))#, err_style='ci_bars')
        arr = np.array(ar)
